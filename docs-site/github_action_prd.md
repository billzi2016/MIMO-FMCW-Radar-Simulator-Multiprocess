...
- 支持静态文档站点的自动发布。
- 支持 `docs-site/docs/zh/` 与 `docs-site/docs/en/` 双语文档目录同时构建。
- 优先兼容 GitHub Pages。
- 工作流结构清晰，便于理解、维护和迁移。
...
- 触发路径必须覆盖 `docs-site/docs/zh/**` 和 `docs-site/docs/en/**`。
- 触发路径必须覆盖 `docs-site/mkdocs.yml`、`docs-site/requirements.txt` 以及文档部署工作流文件本身。
...
- 依赖安装应优先读取 `docs-site/requirements.txt`。
- 依赖来源和安装步骤应便于维护者理解。

### 7.3 构建执行

- 构建命令应直接、清晰、可复现。
- 构建命令应在 `docs-site/` 目录内执行，或显式指定 `docs-site/mkdocs.yml`。
...
- 清晰的构建步骤定义
- 清晰的部署步骤定义
- 与文档工程相匹配的触发规则，至少覆盖 `docs-site/docs/zh/**` 和 `docs-site/docs/en/**`
- 必要的权限配置
- 简要说明文件或注释
...
5. 工作流能完成静态产物上传与部署。
6. 工作流会在中文或英文文档目录变化时触发。
7. 部署目标优先兼容 GitHub Pages。
8. 工作流结构清晰，便于维护者阅读与修改。
9. 工作流具备迁移到其他仓库复用的价值。
...
- 不要漏掉 `docs-site/docs/zh/**` 或 `docs-site/docs/en/**` 任一语言目录的触发规则。
- 不要省略手动触发能力。
- 不要省略最基本的构建与部署链路。
- 不要引入明显超出需求范围的高复杂度设计。
