...
- 文档内容必须从第一版开始按中文和英文分别组织。
- 中文内容统一放在 `docs-site/docs/zh/`。
- 英文内容统一放在 `docs-site/docs/en/`。
- 导航结构在中英文之间尽量保持一致。
...
- 必须同时支持中文和英文。
- 中文和英文文档应采用平行结构组织。
- 初始交付时就必须创建 `zh` 与 `en` 两套内容入口。
- 不允许先完成单语言站点，再通过临时复制、机器翻译或后补目录的方式拼接另一种语言。
- 相同主题的页面在中英文中应尽量一一对应。
- 导航、栏目、层级和命名语义应保持一致。
...
- 文档源码应按语言拆分。
- 中文和英文分别拥有独立的内容入口。
- 中文目录固定为 `docs-site/docs/zh/`。
- 英文目录固定为 `docs-site/docs/en/`。
- 两个目录必须在项目初始化阶段同时创建。
- 每种语言均应具备独立首页。
- 每种语言均应具备对应的导航结构。
...
- 支持语言必须至少包含 `en` 和 `zh`。
- 明确默认语言。
- 默认语言可以是 `en`，但必须保证 `zh` 作为完整入口存在。
- 明确不同语言页面的映射关系或组织方式。
- 组织方式必须基于 `docs/en` 与 `docs/zh` 的目录分层。
- 能支持中文与英文切换。
...
- MkDocs 主配置文件
- 支持 i18n 的多语言配置
- `docs-site/docs/zh/index.md` 中文文档入口页
- `docs-site/docs/en/index.md` 英文文档入口页
- `docs-site/docs/zh/` 与 `docs-site/docs/en/` 下语义对应的基础页面
- 基础栏目对应的占位内容或初始页面
- 静态资源目录
...
5. 中文内容位于 `docs-site/docs/zh/`，英文内容位于 `docs-site/docs/en/`。
6. 中英文首页均可作为独立入口使用。
7. 中英文导航结构基本一致。
8. 站点可通过 GitHub Actions 自动构建。
9. 站点可通过 GitHub Actions 自动部署到 GitHub Pages 或兼容静态托管目标。
...
- 中文内容放置于 `docs-site/docs/zh/`
- 英文内容放置于 `docs-site/docs/en/`
- 从第一版开始同时创建中英文入口和对应页面
- 输出结构清晰、可维护、可迁移
- 提供基础导航和占位页面
